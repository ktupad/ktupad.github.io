...
mymethod:function(){
  ...
  document.getElementById('content').innerHTML=view.blog;


  // document.getElementById('mydiv').innerHTML=id;

  //
  // k[x].ajax.data={mod:'mymethod',myparam:id};
  // k[x].ajax.get(function callback(json){ res=JSON.parse(json);
  // // document.getElementById('content').innerHTML=res;
  // document.getElementById('content').innerHTML=view.mydiv;
...
out='<ul>';
for (i in arr){
  out +='<li><i class="svgIco" onclick="k.blog.app.gourl(\''+arr[i].url+'\')">'+conf.svg(arr[i].ikon)+'</i></li>';
...
if(judul[2]){ var j2='.'+judul[2]; } else {j2=''}
...
// out =k[x].app.sosmed();
document.getElementById('sosmed').innerHTML=k[x].app.sosmed();
...
k[x].modal.view();
  ...
  blogList:function(){
    ...
    if(!el){
// k[x].ajax.path=k[x].app.url.path;
k[x].ajax.data={mod:'bloglist',tb:'blog',id:id};
k[x].ajax.get(callback);
...
// alert(url);
out +='<div class="l-4 s-12 artikel" id="list-'+id+'">';
...
var myClasses = document.getElementsByClassName("code");

for (var i = 0; i < myClasses.length; i++) {
  m=myClasses[i].innerHTML;
  myClasses[i].innerHTML = conf.highlightcode.view(m);
  }
