...
getView:function(id){
out=document.getElementById(id).innerHTML;
return out;
},
...
tbs=conf.appData.addon;
js=[];
for(i in tbs){
js[i]=tbs[i]+'.js';
conf.loadCSS(tbs[i]+'.css');
}
conf.loadScript(js, function() {});
...
view:function (data){
string=data;
string = string.replace(/function|for|return|if/g, '<span style="color:purple;">$&</span>');
string = string.replace(/JSON|json_encode|class|id/g, '<span style="color:orange;">$&</span>');
string = string.replace(/parse|replace|getElementById/g, '<span style="color:blue;">$&</span>');
string = string.replace(/innerHTML|value|document|button|div/g, '<span style="color:red;">$&</span>');
// string = string.replace(/&lt;|&gt;/g, '<span style="color:red;">$&</span>');
// string = string.replace(/(&lt;[^\&]*&gt;)/g, '<span style="color:red;">$1</span>');
// string = string.replace(/\''(.*?)\(/gi, '<span style="color:red;">$&</span>');
